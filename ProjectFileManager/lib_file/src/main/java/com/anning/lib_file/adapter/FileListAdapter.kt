package com.anning.lib_file.adapter
...
import com.anning.lib_file.ChooseFileUIConfig
...
class FileListAdapter(
    private val fileList: MutableList<ChooseFileInfo>,
    private val uiConfig: ChooseFileUIConfig,
) :
    ...
    override fun onBindViewHolder(holder: FileListViewHolder, position: Int) {
        ...
        holder.tvFileName.setTextSize(TypedValue.COMPLEX_UNIT_SP,
            uiConfig.fileNameTextSize.toFloat())

        holder.tvFileSize.text = item.fileSize
        holder.tvFileSize.setTextColor(uiConfig.fileInfoTextColor)
        holder.tvFileSize.setTextSize(TypedValue.COMPLEX_UNIT_SP,
            uiConfig.fileInfoTextSize.toFloat())

        holder.tvFileDate.text = item.fileLastUpdateTime
        holder.tvFileDate.setTextColor(uiConfig.fileInfoTextColor)
        holder.tvFileDate.setTextSize(TypedValue.COMPLEX_UNIT_SP,
            uiConfig.fileInfoTextSize.toFloat())
    }
