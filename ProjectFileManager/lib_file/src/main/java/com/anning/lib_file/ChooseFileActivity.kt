...
import com.anning.lib_file.statusBarHost.StatusBarHost
import com.anning.lib_file.adapter.FileListAdapter
import com.anning.lib_file.adapter.FileNavAdapter
import com.anning.lib_file.detector.ChooseFileHighPolicy
import com.anning.lib_file.detector.ChooseFileLowPolicy
import com.anning.lib_file.utils.PermissionUtil
import com.anning.lib_file.vm.ChooseFileViewModel
import com.anning.lib_file.vm.ChooseFileViewModelFactory
    ...
    private fun popupFileList(fileList: List<ChooseFileInfo>, topInfo: ChooseFileInfo?) {
        ...
        if (fileList.isNotEmpty()) {
            ...
            if (topInfo == null && mViewModel.rootChoosePos > 0) {
                ...
                (rvFiles.layoutManager as LinearLayoutManager).scrollToPositionWithOffset(mViewModel.rootChoosePos,
                    0)
...
//                val chooseFileInfo = ChooseFileInfo()
//                chooseFileInfo.filePath = "/storage/emulated/0"
//                chooseFileInfo.fileName = "0"
//                chooseFileInfo.isDir = true
//                mViewModel.mNavPathList.add(0, chooseFileInfo)
    ...
    private fun initRV() {
        ...
        mViewModel.mFileListAdapter?.setOnFileListClickListener() { position ->
            ...
            if (item.isDir) {
                //设置当前Root的选中
                if (mViewModel.mNavPathList.isEmpty()) {
                    mViewModel.rootChoosePos =
                        (rvFiles.layoutManager as LinearLayoutManager).findFirstVisibleItemPosition()
                }
    ...
    override fun onBackPressed() {
        if (mViewModel.mNavPathList.size == 0) {
            super.onBackPressed()
        } else {
            val lastIndex = mViewModel.mNavPathList.lastIndex;
            mViewModel.mNavPathList.removeAt(lastIndex)
            if (!mViewModel.mNavPathList.isEmpty()) {
                val item = mViewModel.mNavPathList.last()
                mViewModel.mCurPath = item.filePath
                startRefreshAnim()
                obtainByPath(mViewModel.mCurPath)
            }
        ...
        for (chooseFileInfo in mViewModel.mNavPathList) {
            Log.e("TAG", "${chooseFileInfo.filePath}")
        }
        Log.e("TAG", "NavPathList size : ${mViewModel.mNavPathList.size}")
    }
