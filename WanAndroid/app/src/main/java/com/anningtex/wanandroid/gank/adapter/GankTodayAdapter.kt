...
/**
 *
 * @ClassName:      GankTodayAdapter
 * @Description:     java类作用描述
 * @Author:         alvis
 * @CreateDate:     2020/7/6 16:11
 */
class GankTodayAdapter(
    layoutResId: Int,
    sectionHeadResId: Int,
    list: List<GankTodayEntity<GankToday>>
) : BaseSectionQuickAdapter<GankTodayEntity<GankToday>, BaseViewHolder>(
    layoutResId,
    sectionHeadResId,
    list
) {
    ...
    override fun convert(helper: BaseViewHolder?, item: GankTodayEntity<GankToday>?) {
        ...
        helper?.setText(R.id.tv_gank_desc, item?.t?.desc)
            ...
            ?.setGone(
                R.id.ll_gank_images_parent,
                item?.t?.images != null && item.t?.images?.size != 0
            )
