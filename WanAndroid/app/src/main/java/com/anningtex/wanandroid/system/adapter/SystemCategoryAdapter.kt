...
/**
 *
 * @ClassName:      SystemCategoryAdapter
 * @Description:     java类作用描述
 * @Author:         alvis
 * @CreateDate:     2020/7/6 11:45
 */
class SystemCategoryAdapter(layoutResId: Int) :
    BaseQuickAdapter<SystemCategory, BaseViewHolder>(layoutResId) {
    ...
    override fun convert(helper: BaseViewHolder?, item: SystemCategory?) {
        ...
        if (position == mClickedPosition) {
            ...
            helper?.setTextColor(
                R.id.tv_system_category,
                ContextCompat.getColor(mContext, R.color.black_f222)
            )
